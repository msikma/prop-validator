...
describe(`prop-validator package`, () => {
  ...
  describe(`PropTypes`, () => {
    it(`${prefix}.string`, () => {
      ...
      const valid3 = validateProps({ val: PropTypes.string.isRequired }, { val: 'hello' })
      ...
      expect(valid3.results[0]).toMatchObject(validResult('val', ['string']))
    ...
    it(`${prefix}.number`, () => {
      ...
      const valid4 = validateProps({ val: PropTypes.number.isRequired }, { val: -5 })
      ...
      const invalid4 = validateProps({ val: PropTypes.number.isRequired }, { val: null })
      ...
      expect(valid4.results[0]).toMatchObject(validResult('val', ['number']))
      ...
      expect(invalid4.results[0]).toMatchObject(invalidResult('val', ['number'], { message: `Property 'val' should be type 'number', but type 'null' was found` }))
    })
    it(`${prefix}.integer`, () => {
      const valid1 = validateProps({ val: PropTypes.integer }, { val: 2 })
      const valid2 = validateProps({ val: PropTypes.integer }, { val: 6 })
      const valid3 = validateProps({ val: PropTypes.integer.isRequired }, { val: 6 })

      const invalid1 = validateProps({ val: PropTypes.integer }, { val: 2.5 })
      const invalid2 = validateProps({ val: PropTypes.integer }, { val: 'a' })
      const invalid3 = validateProps({ val: PropTypes.integer.isRequired }, { val: null })
      ...
      expect(valid3.results[0]).toMatchObject(validResult('val', ['number: integer']))
      ...
      expect(invalid3.results[0]).toMatchObject(invalidResult('val', ['number: integer'], { message: `Property 'val' should be type 'number: integer', but type 'null' was found` }))
    })
    it(`${prefix}.boolean`, () => {
      ...
      const invalid5 = validateProps({ val: PropTypes.bool.isRequired }, { val: undefined })
      ...
      expect(invalid5.results[0]).toMatchObject(invalidResult('val', ['boolean'], { message: `Property 'val' should be type 'boolean', but type 'undefined' was found` }))
    })
    it(`${prefix}.function`, () => {
    ...
    it(`${prefix}.object`, () => {
      ...
      expect(valid2).toMatchObject(validTypeCheck())
      expect(valid3).toMatchObject(validTypeCheck())
      expect(valid1.results[0]).toMatchObject(validResult('val', ['Object<*>', 'null']))
      expect(valid2.results[0]).toMatchObject(validResult('val', ['Object<*>', 'null']))
      expect(valid3.results[0]).toMatchObject(validResult('val', ['Object<*>']))
      
      expect(invalid1).toMatchObject(invalidTypeCheck())
      expect(invalid1.results[0]).toMatchObject(invalidResult('val', ['Object<*>', 'null'], { message: `Property 'val' should be type 'Object<*> | null', but type 'Array' was found` }))
    })
    it(`${prefix}.array`, () => {
      ...
      const invalid2 = validateProps({ val: PropTypes.array.isRequired }, { val: null })

      expect(valid1).toMatchObject(validTypeCheck())
      expect(valid2).toMatchObject(validTypeCheck())
      expect(valid1.results[0]).toMatchObject(validResult('val', ['Array<*>', 'null']))
      expect(valid2.results[0]).toMatchObject(validResult('val', ['Array<*>']))
      
      expect(invalid1).toMatchObject(invalidTypeCheck())
      expect(invalid2).toMatchObject(invalidTypeCheck())
      expect(invalid1.results[0]).toMatchObject(invalidResult('val', ['Array<*>', 'null'], { message: `Property 'val' should be type 'Array<*> | null', but type 'number' was found` }))
      expect(invalid2.results[0]).toMatchObject(invalidResult('val', ['Array<*>'], { message: `Property 'val' should be type 'Array<*>', but type 'null' was found` }))
    })
    it(`${prefix}.symbol`, () => {
    ...
    it(`${prefix}.null`, () => {
      const valid = [
        validateProps({ val: PropTypes.null }, { val: null }),
        validateProps({ val: PropTypes.null.isRequired }, { val: null }),
        validateProps({ val: PropTypes.null }, { val: undefined })
      ]

      const invalid = [
        validateProps({ val: PropTypes.null }, { val: 1 }),
        validateProps({ val: PropTypes.null.isRequired }, { val: undefined }),
        validateProps({ val: PropTypes.null }, { val: 'b' })
      ]
      ...
      expect(invalid[0].results[0]).toMatchObject(invalidResult('val', ['null'], { message: `Property 'val' should be type 'null', but type 'number' was found` }))
    })
    it(`${prefix}.undefined`, () => {
      const valid = [
        validateProps({ val: PropTypes.undefined }, { val: undefined }),
        validateProps({ val: PropTypes.undefined.isRequired }, { val: undefined })
      ]

      const invalid = [
        validateProps({ val: PropTypes.undefined }, { val: 1 }),
        validateProps({ val: PropTypes.undefined }, { val: null }),
        validateProps({ val: PropTypes.undefined.isRequired }, { val: null }),
        validateProps({ val: PropTypes.undefined }, { val: 'b' })
      ]
      ...
      expect(invalid[0].results[0]).toMatchObject(invalidResult('val', ['undefined'], { message: `Property 'val' should be type 'undefined', but type 'number' was found` }))
    })
    it(`${prefix}.any`, () => {
      const valid = [
        ...
        validateProps({ val: PropTypes.any.isRequired }, { val: null })
      ]
      ...
      for (const validItem of valid) {
        expect(validItem).toMatchObject(validTypeCheck())
        expect(validItem.results[0]).toMatchObject(validResult('val', ['*']))
      }
      
      expect(invalid1).toMatchObject(invalidTypeCheck())
      expect(invalid1.results[0]).toMatchObject(invalidResult('val', ['*'], { message: `Property 'val' should be type '*', but type 'undefined' was found` }))
    })
    it(`${prefix}.numberRange`, () => {
      ...
      expect(() => validateProps({ val: PropTypes.numberRange() }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange(0) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange(1, 2, 3) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.inclusive() }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.inclusive(0) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.inclusive(0, 1, 2) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.exclusive() }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.exclusive(0) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.exclusive(0, 1, 2) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.greaterThan() }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.greaterThan(0, 1) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.greaterThan(0, 1, 2, 3) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.greaterThanOrEqual() }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.greaterThanOrEqual(0, 1) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.greaterThanOrEqual(0, 1, 2, 3) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.lessThan() }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.lessThan(0, 1) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.lessThan(0, 1, 2, 3) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.lessThanOrEqual() }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.lessThanOrEqual(0, 1) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
      expect(() => validateProps({ val: PropTypes.numberRange.lessThanOrEqual(0, 1, 2, 3) }, { val: 3.5 })).toThrow(PropTypesValidatorErrorBase)
    })
    ...
    it(`${prefix}.stringMatching`, () => {
      ...
      const invalid = [
        ...
        validateProps({ val: PropTypes.stringMatching(/^a$/g).isRequired }, { val: null })
      ]
