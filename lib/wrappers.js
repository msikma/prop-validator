...
const makeEnumValidator = validatorConstructor => {
  return (...args) => {
    ...
    if (!validateEnumValues(args)) {
      throw validatorError(`PropTypes.oneOf() only takes a single array of literals that can be checked with Object.is(); received ${debugInvalidArguments(args)}`)
    }
...
const makeMemberValidator = (validatorType, containerValidator, getIterator) => {
  return memberValidator => {
    ...
    if (isArray(memberValidator)) {
      throw validatorError(`PropTypes.${validatorType}() only takes a single PropTypes argument, e.g. PropTypes.${validatorType}(PropTypes.string); received ${debugInvalidArguments(memberValidator)}`)
    }
    if (memberValidator == null) {
      throw validatorError(`PropTypes.${validatorType}() requires a single PropTypes function as argument, e.g. PropTypes.${validatorType}(PropTypes.string); received ${debugInvalidArguments(memberValidator)}`)
    }
...
const makeRegexValidator = (regex) => {
  if (!isRegExp(regex)) {
    throw validatorError(`PropTypes.stringMatching() only takes a single regular expression argument, e.g. PropTypes.stringMatching(/a|b/); received ${debugInvalidArguments(regex)}`)
  }
...
const makeCheckedValidator = (validator, checker, error) => {
  return (...args) => {
    if (!checker(...args)) {
      throw validatorError(`${error}; received ${debugInvalidArguments(...args)}`)
    }
