...
const validatorWrapper = (validatorFunction, valueExpectedTypeObject, canBeNull) => {
  ...
  return isRequired => (value, key, object, state, parentInfo = {}) => {
    const objectPath = [...(state.objectParent || []), key].filter(n => n !== null)
    ...
    const messageData = [valueType, valueExpectedType, valueExpectedTypeList, objectPath, parentInfo]
 ...
 */
const createMessage = (isValid, valueType, valueExpectedType, valueExpectedTypeList, objectPath, parentInfo = {}) => {
  ...
  return `Property '${path}' should be type '${valueExpectedType}'${matchesParent ? `` : `, but type '${valueType}' was found`}`
}
