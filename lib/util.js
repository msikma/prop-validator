...
const joinObjectPath = path => {
  if (!isArray(path)) {
    throw new PropTypesValidatorError(`joinObjectPath() takes an array; received: ${debugInvalidArguments(path)}`)
  }
...
const checkRange = (range, rangeType, validatorName) => {
  ...
  const wrongType = (range[0] != null && !isNumber(range[0])) || (range[1] != null && !isNumber(range[1]))
  if (wrongType) {
    throw new PropTypesValidatorError(`${functionName} arguments must be numbers; received: ${debugInvalidArguments(range)}`)
  }

  if (rangeType === 'inclusive' || rangeType === 'exclusive') {
    const invalidAmount = range.length !== 2
    const wrongOrder = range[0] > range[1]
    if (invalidAmount || wrongOrder) {
      throw new PropTypesValidatorError(`${functionName} arguments must be (min, max); received: ${debugInvalidArguments(range)}`)
    }
  }
  else if (rangeType === 'greaterThan' || rangeType === 'greaterThanOrEqual' || rangeType === 'lessThan' || rangeType === 'lessThanOrEqual') {
    const invalidAmount = range.length !== 1
    if (invalidAmount) {
      throw new PropTypesValidatorError(`${functionName} argument must be a single number; received: ${debugInvalidArguments(range)}`)
    }
  }
  else {
    throw new PropTypesValidatorError(`${functionName} was called with an invalid range type: ${rangeType}`)
  }
