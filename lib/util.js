...
const checkRange = (range) => {
  ...
  const noThreeN = range.length === 3 && !isString(range[1])
  const noTwoN = range.length === 2 && (!isString(range[1]) && !isString(range[0]))
  const minAfterMax = range.length === 3 && range[0] > range[2]
  
  if (tooShort || noThreeN || noTwoN || minAfterMax) {
    throw validatorError(`PropTypes.{integer,number}Range() arguments must be [min, 'n'], [min, 'n', max] or ['n', max]; received: ${JSON.stringify(range)}`)
  }
  return true
}
...
const inRange = (range, number) => {
  ...
  // Check min < n < max
  if (range[1] === 'n' && range.length === 3) {
    return range[0] < number && number < range[2]
  }
  // Check min < n
  if (range[1] === 'n' && range.length === 2) {
