...
const checkRange = (range, rangeType, validatorName) => {
  ...
  if (rangeType === 'inclusive' || rangeType === 'exclusive') {
    ...
    if (tooShort || wrongOrder) {
      throw validatorError(`${functionName} arguments must be (min, max); received: ${JSON.stringify(range)}`)
    }
  }
    ...
    if (tooLong) {
      throw validatorError(`${functionName} argument must be a single number; received: ${JSON.stringify(range)}`)
    }
  }
  else {
